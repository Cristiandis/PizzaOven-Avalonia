...
public class PLUSRonnieAnimate
{
    ...
    public double GlideX { get; set; }

    public double GlideY { get; set; }
    ...
    public void Destroy()
    {
        ...
        {
            if (_overlayCanvas.Parent is Panel parent) parent.Children.Remove(_overlayCanvas);
    ...
    {
        var currentX = Canvas.GetLeft(_image);
        var currentY = Canvas.GetTop(_image);

        var dx = GlideX - currentX;
        var dy = GlideY - currentY;

        if (Math.Abs(dx) < 1 && Math.Abs(dy) < 1)
        {
            Canvas.SetLeft(_image, GlideX);
            Canvas.SetTop(_image, GlideY);
            _timer.Stop();
            return;
        }

        var dist = Math.Sqrt(dx * dx + dy * dy);
        var moveX = dx / dist * _speed;
        var moveY = dy / dist * _speed;
    ...
    public async Task WaitForClickOnImageAsync()
    {
        ...
        void Handler(object? sender, PointerPressedEventArgs e)
        {
            ...
            var x = Canvas.GetLeft(_image);
            var y = Canvas.GetTop(_image);

            var w = _image.Bounds.Width;
            var h = _image.Bounds.Height;
    ...
    public static double MeasureTextBlockHeight(string text, double boxWidth = 373, double sidePadding = 35,
        double fontSize = 21, double topHeight = 19, double bottomHeight = 19)
    {
        ...
        var middleCount = (int)Math.Ceiling(textHeight);

        var totalHeight = topHeight + middleCount + bottomHeight;
    ...
    public int MakeTextbox(double x, double y, string text)
    {
        ...
        var totalMiddleHeight = textHeight;
        var middleCount = (int)Math.Ceiling(totalMiddleHeight);
    ...
    public async Task MakeSkipButtonAsync(Canvas parent, Action onClickAction)
    {
        ...
        skipButton.Click += (_, __) => { tcs.TrySetResult(true); };

        await Task.Delay(1000);

        var finishedTask = await Task.WhenAny(
            tcs.Task,
            WaitForClickOnImageAsync()
        );
